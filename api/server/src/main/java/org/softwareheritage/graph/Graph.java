...
public class Graph {
  ...
  private void setupNodesMapping() throws IOException, ClassNotFoundException {
    ...
    InputStream nodeFile = new GZIPInputStream(new FileInputStream(path + ".nodes.csv.gz"));
    Collection<MutableString> hashes =
        new LineIterator(new FastBufferedReader(new InputStreamReader(nodeFile, "UTF-8")))
            .allLines();
