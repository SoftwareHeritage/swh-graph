...
def run_e2e_check(
    ...
    # It includes a few projects, such as `parmap`, `flask`, or `swh-graph` itself.
    # They should not change in the future, as all traversals start from fixed origins.
    # The keys of this dictionary are the origins of the aforementioned projects.
    # For each origin, there is a list of SWHIDs that correspond to revisions,
    # directories or content related to this origin.
    ...
    check_authors = sensitive_out_dir is not None
    ...
    if check_authors:
        assert Path(f"{sensitive_out_dir}/{graph_name}.persons").exists()
        assert Path(f"{sensitive_out_dir}/{graph_name}.persons.ef").exists()
        ...
        logger.info("End-to-end checks for full names skipped")
    ...
    for origin, swhids in projects.items():
        for swhid in swhids:
            full_swhid = attr.evolve(QualifiedSWHID.from_string(swhid), origin=origin)
