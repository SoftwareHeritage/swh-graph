...
def test_download_graph_ko(
    cli_runner, s3_graph_dataset_name, mocker, tmp_path, mocked_aws
):
    mocker.patch("swh.graph.download.GraphDownloader.download_graph").return_value = (
        False
    )
    result = cli_runner.invoke(
        graph_cli_group,
    ...
    assert (
        f"Graph dataset {s3_graph_dataset_name} partially downloaded to path {tmp_path}."
    ) in result.output
